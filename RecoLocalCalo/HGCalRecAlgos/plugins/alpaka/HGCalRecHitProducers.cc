      ...
      if (calibWatcher_.check(iSetup)) {
        for (int i = 0; i < deviceConfigParamProvider.view().metadata().size(); i++) {
          log << "idx = " << i << ", "
              ...
              << "BXm1_slope = " << hostCalibParamProvider.view()[i].BXm1_slope() << ", " << std::endl;
        }
      }
    });
...
#else
    auto recHits = calibrator_.calibrate(queue, hostDigis, deviceCalibParam, deviceConfigParamProvider, deviceMappingCellParamProvider, deviceIndexingParamProvider);
#endif
